...
The AI code agent must **proactively detect and leverage all MCP (Model Context Protocol) tools integrated into the Cursor environment**, whenever their use is beneficial or required for task success.
...
1. **Tool Awareness**
   - At the beginning of each task, the agent must determine which MCP tools are available in the current Cursor environment.
