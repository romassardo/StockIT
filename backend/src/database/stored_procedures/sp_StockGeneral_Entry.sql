...
-- Description: Registrar entrada de stock (para productos sin número de serie)
-- Version: 1.0.41 - Versión final con StrictMode deshabilitado
-- =============================================
USE StockIT;
GO

-- Verificar si el procedimiento ya existe y eliminarlo
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_StockGeneral_Entry')
BEGIN
    DROP PROCEDURE sp_StockGeneral_Entry;
    PRINT N'Procedimiento sp_StockGeneral_Entry eliminado para su recreación.';
END
GO
...
PRINT N'Procedimiento sp_StockGeneral_Entry creado exitosamente (VERSIÓN FINAL).';
GO
