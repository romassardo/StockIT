...
BEGIN
    ...
    SELECT
        ...
        ISNULL(p.stock_minimo, 0) AS min_stock, -- Corregido de p.min_stock a p.stock_minimo
        ...
        (CASE WHEN sg.cantidad_actual < ISNULL(p.stock_minimo, 0) THEN 1 ELSE 0 END) AS alerta_stock_bajo -- Usar p.stock_minimo
    FROM 
        StockGeneral sg
    INNER JOIN 
    ...
    -- ELIMINADO INNER JOIN con la tabla Marca
    WHERE
        p.activo = 1
        ...
        AND (@solo_bajo_stock = 0 OR sg.cantidad_actual < ISNULL(p.stock_minimo, 0)) -- Usar p.stock_minimo
    ORDER BY
        p.nombre ASC;
END
GO
